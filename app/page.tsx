            ...
            {config.heroDisplayMode === 'logo' ? (
              <div className="lg:col-span-5 flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8 select-none animate-fadeIn">
                <div className="relative flex flex-col items-center justify-center group w-full max-w-[460px]">
                  ...
                  {/* Logotipo da Empresa em Tamanho Maior (Estático) */}
                  <img
                    src={config.heroCustomImageUrl || '/diamond-relics-logo.png'}
                    alt="Diamond Relics"
                    className="relative z-10 w-full max-w-[340px] sm:max-w-[400px] md:max-w-[450px] h-auto object-contain drop-shadow-[0_15px_40px_rgba(0,0,0,0.9)] select-none pointer-events-none"
                  />

                  {/* Identificador Oficial do Instagram abaixo do logotipo (Link Clicável) */}
                  {config.heroCustomInstagramHandle?.trim() && (
                    <a
                      href={`https://instagram.com/${config.heroCustomInstagramHandle.replace('@', '').trim()}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="relative z-10 mt-6 flex items-center justify-center gap-2.5 px-4 py-2 rounded-full bg-[#12151B]/90 hover:bg-[#1A1E26] border border-[#282E3A] hover:border-[#f2ca50] shadow-xl backdrop-blur-md transition-all group/insta cursor-pointer"
                      title={`Seguir ${config.heroCustomInstagramHandle} no Instagram`}
                    >
                      <svg
                        className="w-4 h-4 text-[#f2ca50] group-hover/insta:scale-110 transition-transform shrink-0"
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                      ...
                      <span className="text-xs font-['Space_Grotesk'] text-[#F4F1EA] group-hover/insta:text-[#f2ca50] font-medium tracking-wide transition-colors">
                        {config.heroCustomInstagramHandle}
                      </span>
                      <span className="material-symbols-outlined text-[13px] text-[#9CA3AF] group-hover/insta:text-[#f2ca50] transition-colors">
                        open_in_new
                      </span>
                    </a>
                  )}
